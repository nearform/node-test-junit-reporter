...
test('validates built report data', async () => {
  const reportData = await buildReportData(mockedSource)

  // Total duration from diagnostic events
  assert.strictEqual(reportData.duration, 438.0355)

  // Number of test suites
  assert.strictEqual(reportData.testSuites.length, 3)

  const [brokenSuite, nestedSuite, skippedSuite] = reportData.testSuites
